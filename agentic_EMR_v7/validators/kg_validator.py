...
class KGValidator:
    ...
    #             continue
    #
    #         entity_text = self._normalize_text(self._collect_entity_text(ent))
    #
    ...
    #             ]
    #
    #             if any(keyword and keyword in entity_text for keyword in normalized_keywords):
    #                 issues.append({
    #                     "source": "kg",
    ...
    def _infer_field_from_slot(self, slot: str) -> str:
        ...
        if any(x in slot_norm for x in ["诱因", "饮食", "旅行史", "用药史", "有关", "相关", "受凉", "进食油腻", "接触史", "基础病"]):
            return "inducement"

        if any(x in slot_norm for x in ["缓解", "加重", "体位", "饭后", "进食后", "平卧", "弯腰", "排便后", "休息后"]):
            return "alleviating_factors"

        if any(x in slot_norm for x in ["进食", "食物", "餐后", "空腹", "饥饿"]):
    ...
    #
    #     if "部位" in slot_norm or "范围" in slot_norm or "侧" in slot_norm:
    #         extra_field_candidates.append("location")
    #
    #     if any(x in slot_norm for x in ["持续", "阵发", "规律", "昼夜", "夜间", "白天", "进行性", "逐渐", "波动"]):
    #         extra_field_candidates.extend(["duration_pattern", "progression", "frequency"])
    #
    #     if any(x in slot_norm for x in ["程度", "严重", "量", "大小", "影响", "减轻多少", "凹陷性"]):
    #         extra_field_candidates.append("severity")
    #
    #     if any(x in slot_norm for x in ["伴", "有无", "是否伴", "是否有", "是否出现", "是否存在"]):
    #         extra_field_candidates.extend(["associated_symptoms", "negative_symptoms"])
    #
    #     if any(x in slot_norm for x in ["进食", "食物", "餐后", "空腹", "饥饿"]):
    #         extra_field_candidates.extend(["relation_to_food", "alleviating_factors"])
    #
    #     if any(x in slot_norm for x in ["排便", "排气", "便意", "便后", "里急后重", "失禁"]):
    #         extra_field_candidates.extend(["relation_to_bowel", "alleviating_factors"])
    #
    #     if any(x in slot_norm for x in ["体位", "平卧", "弯腰", "坐起", "站立", "不能平卧"]):
    #         extra_field_candidates.extend(["relation_to_position", "alleviating_factors"])
    #
    #     if any(x in slot for x in ["晨起", "饭后", "空腹"]):
    #         extra_field_candidates.extend([
    #             "relation_to_food",
    #             "inducement",
    #             "alleviating_factors",
    #             "characteristics",
    #         ])
    #
    #     if any(x in slot for x in ["无法进食", "不能进食", "进食"]):
    #         extra_field_candidates.extend([
    #             "severity",
    #             "associated_symptoms",
    #             "negative_symptoms",
    #         ])
    #
    #     if any(x in slot for x in ["尿少", "脱水", "口渴"]):
    #         extra_field_candidates.extend([
    #             "negative_symptoms",
    #             "associated_symptoms",
    #             "severity",
    #         ])
    #
    #     for field_name in extra_field_candidates:
    ...
    #     if slot_main_norm and slot_main_norm in entity_text:
    #         return True
    #
    #     return False
    def _slot_is_filled(self, ent: Dict, slot: str) -> bool:
