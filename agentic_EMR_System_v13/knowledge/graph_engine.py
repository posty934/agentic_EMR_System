...
class MedicalGraph:
    ...
    def _slot_constraint_rule_id(self, symptom_name: str, slot_name: str, idx: int) -> str:
        digest = abs(hash((symptom_name, slot_name, idx)))
        return f"slot_constraint_rule:{symptom_name}:{digest}"
    ...
    def _build_ontology_graph(self) -> None:
        ...
        for idx, rule in enumerate(self.logic_rules.get("slot_answer_constraints", [])):
            symptom = str(rule.get("symptom") or "").strip()
            slot = str(rule.get("slot") or "").strip()
            if not symptom or not slot:
                continue

            symptom_id = self._ensure_symptom_node(symptom)
            slot_id = self._ensure_slot_node(slot)
            rule_id = self._slot_constraint_rule_id(symptom, slot, idx)

            self.graph.add_node(
                GraphNode(
                    id=rule_id,
                    kind="slot_answer_constraint_rule",
                    label=f"{symptom}_{slot}_answer_constraint_{idx}",
                    props={
                        "symptom": symptom,
                        "slot": slot,
                        "allowed_keywords": list(rule.get("allowed_keywords", [])),
                        "conflict_keywords": list(rule.get("conflict_keywords", [])),
                        "message": rule.get("message", ""),
                        "rollback_question": rule.get("rollback_question", ""),
                    },
            ...
            self.graph.add_edge(
                GraphEdge(
                    source=symptom_id,
                    relation="HAS_SLOT_ANSWER_CONSTRAINT",
                    target=rule_id,
                )
            )
            self.graph.add_edge(
    ...
    def get_slot_answer_constraint_rules(self, symptom_name: str) -> List[Dict[str, Any]]:
        symptom_id = self._symptom_id(symptom_name)
        rules: List[Dict[str, Any]] = []
        for node in self.graph.neighbors(symptom_id, "HAS_SLOT_ANSWER_CONSTRAINT"):
            rules.append(dict(node.props))
        return rules
