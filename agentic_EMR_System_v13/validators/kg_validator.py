...
from knowledge.case_graph_builder import CaseGraphBuilder
...
class KGValidator:
    def __init__(self):
        ...
        # 这里保留你现有的 slot grounding 逻辑，不丢任何兼容性
        self.case_graph_builder = CaseGraphBuilder(
            ontology=self.graph,
            slot_resolver=self._slot_is_filled,
            symptom_resolver=self._get_symptom_term,
            entity_text_collector=self._collect_entity_text,
            slot_value_resolver=self._resolve_slot_value,
        )

        # ===== 下面这两个大字典，继续保留你当前 v11 的原始内容，不要改 =====
        self.slot_field_map = {
            # 这里继续放你当前 v11 的完整 slot_field_map
        }

        self.slot_keywords = {
            # 这里继续放你当前 v11 的完整 slot_keywords
        }
        ...
        self.slot_keywords = {
            "诱因(不洁饮食/受凉)": [
                "不洁饮食", "饮食不洁", "吃坏东西", "不干净食物", "食物不洁",
                "受凉", "着凉", "腹部受凉", "辛辣", "火锅", "刺激性食物"
            ],
            "诱因(饮水少/膳食纤维少/活动减少/药物)": [
                "饮水少", "喝水少", "喝水很少", "水喝得少",
                "膳食纤维少", "纤维少", "蔬菜少", "水果少",
                "活动减少", "运动少", "久坐", "药物", "吃药"
            ],
            "大便的性状(水样便/黏液便/脓血便)": [
                "水样便", "稀水样", "稀便", "黏液便", "粘液便", "脓血便"
            ],
            "大便的性状(干硬便/球状便)": [
                "干硬便", "干硬", "硬便", "大便干", "球状便", "颗粒便", "羊粪球"
            ],
            "具体部位": [
    ...
    def validate(self, current_entities: List[Dict], draft: Dict) -> Dict:
        case_graph = self.case_graph_builder.build(
            current_entities=current_entities or [],
            draft=draft or {},
        )

        query_engine = GraphQueries(
            ontology=self.graph,
            case_graph=case_graph,
            conflict_matcher=self._contains_conflict_keyword,
        )

        issues = query_engine.run_all()

        kg_pass = len([x for x in issues if x.get("severity") == "high"]) == 0
    ...
    def _contains_conflict_keyword(self, entity_text: str, keyword: str) -> bool:
        raw_entity_text = str(entity_text or "")
        entity_text = self._normalize_text(raw_entity_text)
        ...
        # 1. 先做直接子串匹配
        if keyword_norm in entity_text and not self._keyword_is_negated(raw_entity_text, keyword):
            return True

        # 2. 再做常见语序/表达变体匹配
        for variant in self._expand_conflict_keyword_variants(keyword):
            variant_norm = self._normalize_text(variant)
            if variant_norm and variant_norm in entity_text and not self._keyword_is_negated(raw_entity_text, variant):
                return True

        if self._keyword_is_negated(raw_entity_text, keyword):
            return False
    ...
    def _expand_conflict_keyword_variants(self, keyword: str) -> List[str]:
        variant_map = {
            "水样便": ["稀水样便", "水一样大便", "拉水", "拉稀水", "水便"],
            "稀便": ["大便稀", "稀烂便", "稀糊便", "不成形便", "糊状便"],
            "黏液便": ["粘液便", "带黏液", "带粘液"],
    ...
    def _keyword_is_negated(self, text: str, keyword: str) -> bool:
        ...
        if not raw_text or not raw_keyword:
            return False

        negation_words = ["无", "没有", "没", "未", "否认", "不是", "不为", "并非", "不伴", "未伴"]
        for match in re.finditer(re.escape(raw_keyword), raw_text):
            prefix = raw_text[max(0, match.start() - 8):match.start()]
            if any(word in prefix for word in negation_words):
                return True

        keyword_norm = self._normalize_text(raw_keyword)
        text_norm = self._normalize_text(raw_text)
        for word in negation_words:
            word_norm = self._normalize_text(word)
            if keyword_norm and f"{word_norm}{keyword_norm}" in text_norm:
                return True

        return False

    def _field_needs_slot_keyword_match(self, field_name: str, slot: str) -> bool:
        if field_name != "characteristics":
            return False

        slot_norm = self._normalize_text(slot)
        return any(
            keyword in slot_norm
            for keyword in [
                "性状",
                "性质",
                "颜色",
    ...
    def _resolve_slot_value(self, ent: Dict, slot: str) -> Dict[str, str]:
        ...
        raw_answer = str(slot_answers.get(slot, "") or "").strip()
        display_answer = str(slot_display_answers.get(slot, "") or "").strip()
        if raw_answer or display_answer:
            return {
                "answer": raw_answer,
        ...
        field = self._infer_field_from_slot(slot)
        if field:
            value = ent.get(field)
            if value is not None and str(value).strip() != "":
                return {
        ...
        if isinstance(dynamic_details, dict):
            for dk, dv in dynamic_details.items():
                ...
                if dk_norm and (dk_norm == slot_norm or dk_norm in slot_norm or slot_norm in dk_norm):
        ...
        for field_name in dedup_fields:
            ...
            if field_name in strict_match_fields:
                if self._field_value_matches_slot(value, slot):
                    return True
            elif self._field_needs_slot_keyword_match(field_name, slot):
                if self._field_value_matches_slot(value, slot):
                    return True
            else:
                return True
        ...
        if isinstance(dynamic_details, dict):
            for dk, dv in dynamic_details.items():
                ...
                if dk_norm == slot_norm or dk_norm in slot_norm or slot_norm in dk_norm:
                    if self._field_needs_slot_keyword_match("characteristics", slot):
                        return self._field_value_matches_slot(dv, slot)
                    return True
    ...
    def _collect_entity_text(self, ent: Dict) -> str:
        ...
        for slot, answer in slot_answers.items():
            if answer is None or str(answer).strip() == "":
                continue
            mapped_field = self._infer_field_from_slot(str(slot or ""))
            if mapped_field:
                shadowed_fields.add(mapped_field)
