...
7. 除了 COMPLETED 之外，输出内容必须是患者可见话术，而不是后台任务描述。"""),
        ...
        self.slot_display_prompt = ChatPromptTemplate.from_messages([
            ("system", """你是一名医疗问诊信息展示助手。

        你的任务：
        把“问诊要点 + AI实际追问 + 患者回答”改写成一句短的、直观的问诊记录，用于前端展示。

        【最高优先级规则】
        1. 必须严格保留患者回答中的所有有效细节，尤其是：
           - 程度：有点、轻微、明显、很严重、偶尔、经常、一点点、一些、很多
           - 主观性：感觉、觉得、好像、疑似、不确定
           - 频率/时间/次数：几天一次、每天几次、持续多久、饭后、夜间
           - 数量/范围：少量、一些、大量、范围、部位
        2. 不允许把“有点/一点点/轻微”直接省略成“有”。
        3. 不允许把“感觉有一些”改成确定性的“有”，必须保留“感觉/一些”等限定。
        4. 不允许新增患者没有表达的严重程度、诊断、病因或医学判断。
        5. 如果患者回答是单纯“有/是/会/对”，才可以结合问诊要点写成“有XXX情况”。
        6. 如果患者回答是单纯“没有/无/不会/否”，才可以写成“无XXX情况”或“未见XXX”。
        7. 如果患者回答带有程度或限定词，输出中必须体现这些词的含义。
        8. 输出一句陈述句，不要输出问句。
        9. 输出尽量控制在 8 到 35 个中文字符。
        10. 不要加引号，不要加前缀，不要解释。
        ...
        self.slot_display_chain = self.slot_display_prompt | self.llm | StrOutputParser()
    ...
    def _build_history_pending_target(self, targets: List[dict]) -> dict:
        target_payloads = []
        for ent in targets or []:
            symptom_name = (ent.get("symptom_name") or "").strip()
            standard_term = (ent.get("standard_term") or symptom_name).strip()
            if not symptom_name and not standard_term:
                continue
        ...
        primary = target_payloads[0]
        return {
            "type": "history_comparison",
            "symptom_name": primary.get("symptom_name", ""),
            "standard_term": primary.get("standard_term", ""),
            "targets": target_payloads,
        }

    def _ensure_entity_meta(self, ent: dict) -> dict:
        ent.setdefault("slot_answers", {})
        ent.setdefault("slot_display_answers", {})
    ...
    def _fallback_slot_display_answer(self, slot: str, answer: str) -> str:
        raw_answer = str(answer or "").strip()
        compact_slot = self._compact_slot_text(slot)
        compact_slot = compact_slot.replace("是否", "").replace("有无", "")
        compact_slot = compact_slot.strip("：:，。！？?呢吗 ")

        if not raw_answer:
            return ""

        subjective_prefix = ""
        if any(x in raw_answer for x in ["感觉", "觉得", "好像", "疑似", "不确定"]):
            subjective_prefix = "感觉"

        mild_words = ["有点", "一点点", "轻微", "稍微", "略微"]
        obvious_words = ["明显", "比较明显", "很", "严重", "厉害"]
        small_amount_words = ["一些", "少量", "一点"]

        if self._is_negative_answer(raw_answer):
            return f"无{compact_slot}情况"

        if any(x in raw_answer for x in mild_words):
            return f"{subjective_prefix}有轻微{compact_slot}".strip()

        if any(x in raw_answer for x in obvious_words):
            return f"{subjective_prefix}有明显{compact_slot}".strip()

        if any(x in raw_answer for x in small_amount_words):
            return f"{subjective_prefix}有少量{compact_slot}".strip()

        if self._is_affirmative_answer(raw_answer):
            return f"有{compact_slot}情况"

        return raw_answer

    def _build_slot_display_answer(
        self,
        slot: str,
        answer: str,
        question: str = "",
        ...
        fallback = self._fallback_slot_display_answer(slot, answer)

        try:
            raw = self.slot_display_chain.invoke({
                "slot": slot,
                "question": question or slot,
                "answer": answer,
            ...
            if self.debug:
                print(f"[SlotDisplay] 生成展示文案失败: {e}")
            return fallback
            ...
            if value is not None and str(value).strip() != "":
                if field in strict_match_fields:
                    if self.kg_validator._field_value_matches_slot(value, slot):
                        return str(value).strip()
                elif self.kg_validator._field_needs_slot_keyword_match(field, slot):
                    if self.kg_validator._field_value_matches_slot(value, slot):
                        return str(value).strip()
                else:
                    return str(value).strip()
        ...
        if isinstance(dynamic_details, dict):
            for dk, dv in dynamic_details.items():
                ...
                if dk_norm == slot_norm or dk_norm in slot_norm or slot_norm in dk_norm:
                    if self.kg_validator._field_needs_slot_keyword_match("characteristics", slot):
                        if self.kg_validator._field_value_matches_slot(dv, slot):
                            return str(dv).strip()
                        continue
                    return str(dv).strip()
    ...
    def _relocate_misfiled_slot_answers(self, ent: dict, slots: List[str]) -> dict:
        slot_answers = ent.get("slot_answers", {})
        if not isinstance(slot_answers, dict) or not slot_answers:
            return ent

        slot_display_answers = ent.get("slot_display_answers", {})
        if not isinstance(slot_display_answers, dict):
            slot_display_answers = {}
            ent["slot_display_answers"] = slot_display_answers

        for slot, answer in list(slot_answers.items()):
            slot = str(slot or "").strip()
            answer_text = str(answer or "").strip()
            if not slot or not answer_text:
                continue
            if not self.kg_validator._field_needs_slot_keyword_match("characteristics", slot):
                continue
            if self.kg_validator._field_value_matches_slot(answer_text, slot):
                continue

            for target_slot in slots:
                if target_slot == slot:
                    continue
                if not self.kg_validator._field_value_matches_slot(answer_text, target_slot):
                    continue

                if not str(slot_answers.get(target_slot, "") or "").strip():
                    slot_answers[target_slot] = answer_text
                slot_answers.pop(slot, None)
                slot_display_answers.pop(slot, None)
                break
    ...
    def refresh_entities_slot_state(self, current_entities: List[dict]) -> List[dict]:
        ...
        for ent in current_entities or []:
            ...
            slots = self._get_guideline_slots_for_symptom(standard_term)
            copied = self._relocate_misfiled_slot_answers(copied, slots)
    ...
    def _build_forced_slot_update(
        ...
        display_answer = self._build_slot_display_answer(
            slot=slot,
            answer=answer,
            question=last_ai_message,
            symptom_name=pending_question_target.get("symptom_name", "")
        )
    ...
    def plan_next_turn(
        ...
        if history_targets and not self._all_history_targets_completed(active_entities, long_term_memory_str):
            return {
                ...
                "pending_question_target": self._build_history_pending_target(history_targets),
                "entities": current_entities,
            }
    ...
    def _build_fallback_slot_update(
        ...
        display_answer = self._build_slot_display_answer(
            ...
            answer=answer,
            question=last_ai_message,
            symptom_name=question_target.get("symptom_name", "")
        )
    ...
    def _build_history_relation_updates_from_pending(
        self,
        current_entities: List[dict],
        ...
        pending_targets = pending_question_target.get("targets")
        if not isinstance(pending_targets, list) or not pending_targets:
            pending_targets = [pending_question_target]
        ...
        updates = []
        for ent in self._get_active_entities(current_entities or []):
            if not any(self._target_matches_entity(target, ent) for target in pending_targets):
                continue
    ...
    def extract(
        ...
        try:
            ...
            if pending_question_target.get("type") == "history_comparison":
                targeted_updates = self._build_history_relation_updates_from_pending(
