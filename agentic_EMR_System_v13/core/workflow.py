...
class EMRWorkflow:
    ...
    def _target_matches_entity(self, target: dict, ent: dict) -> bool:
        ...
        return bool(
            (target_symptom and target_symptom in {symptom_name, standard_term})
            or (target_standard and target_standard in {symptom_name, standard_term})
        )

    def _is_forced_slot_update(self, pending_question_target: dict, new_ent: dict, existing: dict) -> bool:
        if (pending_question_target or {}).get("type") != "guideline_slot":
            return False
        if not self._target_matches_entity(pending_question_target, existing):
            return False

        target_slot = (pending_question_target.get("slot") or "").strip()
        if not target_slot:
            return False

        slot_answers = new_ent.get("slot_answers", {})
        return isinstance(slot_answers, dict) and target_slot in slot_answers

    def _is_quality_reask_slot_update(self, pending_question_target: dict, new_ent: dict, existing: dict) -> bool:
        if (pending_question_target or {}).get("source") != "quality_reask":
            return False
        return self._is_forced_slot_update(pending_question_target, new_ent, existing)

    def _is_history_comparison_update(self, pending_question_target: dict, new_ent: dict, existing: dict) -> bool:
        if (pending_question_target or {}).get("type") != "history_comparison":
            return False

        targets = pending_question_target.get("targets")
        if isinstance(targets, list) and targets:
            target_matched = any(self._target_matches_entity(target, existing) for target in targets)
        else:
            target_matched = self._target_matches_entity(pending_question_target, existing)

        if not target_matched:
            return False

        dynamic_details = new_ent.get("dynamic_details", {})
        return (
            isinstance(dynamic_details, dict)
            and str(dynamic_details.get("与过往病史关联", "")).strip() != ""
        )

    def _ensure_slot_review_status(self, ent: dict) -> dict:
        status = ent.get("slot_review_status", {})
        if not isinstance(status, dict):
            status = {}
        ent["slot_review_status"] = status
        return status

    def _mark_slot_invalid(self, ent: dict, target: dict, rollback_question: str = ""):
        slot = (target.get("slot") or "").strip()
        if not slot:
            return

        review_status = self._ensure_slot_review_status(ent)
        review_status[slot] = {
            "status": "invalid",
            "reason": (target.get("reason") or "该回答与当前症状或问诊要点不匹配，需要重新确认。").strip(),
            "current_answer": (target.get("current_answer") or "").strip(),
            "rollback_question": (rollback_question or "").strip(),
        }
        ent["is_locked"] = False

    def _mark_slot_reasked(self, ent: dict, slot: str, answer: str):
        slot = (slot or "").strip()
        if not slot:
            return

        review_status = self._ensure_slot_review_status(ent)
        previous = review_status.get(slot, {}) if isinstance(review_status.get(slot), dict) else {}
        review_status[slot] = {
            "status": "reasked",
            "reason": previous.get("reason", ""),
            "previous_answer": previous.get("current_answer", ""),
            "updated_answer": str(answer or "").strip(),
        }

    def _overwrite_slot_answer(self, ent: dict, slot: str, answer: str):
        slot = (slot or "").strip()
        ...
        ent["slot_answers"][slot] = answer

        mapped_field = self.agent1.kg_validator._infer_field_from_slot(slot)
        if mapped_field:
            ent[mapped_field] = answer

        dynamic_details = ent.get("dynamic_details", {})
        if isinstance(dynamic_details, dict):
            slot_norm = self.agent1.kg_validator._normalize_text(slot)
            for key in list(dynamic_details.keys()):
                key_norm = self.agent1.kg_validator._normalize_text(key)
                if key_norm and (key_norm == slot_norm or key_norm in slot_norm or slot_norm in key_norm):
                    dynamic_details.pop(key, None)

    def _apply_quality_issue_marks(
        self,
        entities: list,
        ...
        for ent in entities or []:
            copied = dict(ent or {})
            copied.setdefault("slot_answers", {})
            copied.setdefault("slot_display_answers", {})
            copied.setdefault("dynamic_details", {})
            self._ensure_slot_review_status(copied)

            for target in invalid_targets:
                if not self._target_matches_entity(target, copied):
                    continue

                self._mark_slot_invalid(copied, target, rollback_question=rollback_question)
                if not pending_target:
                    pending_target = {
                        "type": "guideline_slot",
                        "symptom_name": (copied.get("symptom_name") or "").strip(),
                        "standard_term": (copied.get("standard_term") or copied.get("symptom_name") or "").strip(),
                        "slot": (target.get("slot") or "").strip(),
                        "source": "quality_reask",
                    }

            updated_entities.append(copied)

        return updated_entities, pending_target
    ...
    def node_extractor(self, state: EMRState):
        ...
        if extracted_data:
            ...
            for new_ent in extracted_data:
                ...
                if existing:
                    is_forced_slot_update = self._is_forced_slot_update(
                        pending_question_target=pending_question_target,
                        new_ent=new_ent,
                        existing=existing
                    )
                    is_quality_reask_slot_update = self._is_quality_reask_slot_update(
                        pending_question_target=pending_question_target,
                        new_ent=new_ent,
                        existing=existing
                    )
                    is_history_comparison_update = self._is_history_comparison_update(
                        pending_question_target=pending_question_target,
                        new_ent=new_ent,
                        existing=existing
                    )
                    ...
                    if (
                        existing.get("is_locked", False)
                        and not is_forced_slot_update
                        and not is_history_comparison_update
                    ):
                        continue
                    ...
                    self._ensure_slot_review_status(existing)
                    ...
                    for slot, answer in (new_ent.get("slot_answers", {}) or {}).items():
                        ...
                        if is_forced_slot_update and slot == pending_question_target.get("slot"):
                            self._overwrite_slot_answer(existing, slot, answer)
                            if is_quality_reask_slot_update:
                                self._mark_slot_reasked(existing, slot, answer)
                        else:
    ...
    def node_reviewer(self, state: EMRState):
        ...
        marked_entities = entities
        quality_pending_target = {}
        ...
        if not validation["final_pass"]:
            if auto_revision_possible:
                ...
                marked_entities, quality_pending_target = self._apply_quality_issue_marks(
                    entities=entities,
                    invalid_targets=validation.get("invalid_targets", []),
                    rollback_question=validation.get("rollback_question", "")
                )
        ...
        return {
            ...
            "pending_question_target": quality_pending_target or state.get("pending_question_target", {})
        }
