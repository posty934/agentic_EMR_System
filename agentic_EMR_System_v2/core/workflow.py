...
class EMRWorkflow:
    ...
    def node_extractor(self, state: EMRState):
        ...
        extracted_data = self.agent1.extract(last_human_msg, entities, last_ai_msg)
        ...
        # 🚨 注意这里增加了 last_ai_msg 参数
        reply = self.agent1.generate_reply(last_human_msg, updated_entities, last_ai_msg)
    ...
    def node_reviewer(self, state: EMRState):
        ...
        if not validation["is_valid"]:
            # 🚨 修改点 1：把报警文本和追问话术拆成两条独立的消息！
            # 第一条消息：带有红灯前缀，会被 main_app 渲染成红色拦截框
            warning_text = f"🚨 **【系统质控拦截】** 🚨\n\n**发现医学逻辑矛盾：** {validation.get('feedback')}"
            new_messages.append(AIMessage(content=warning_text))

            # 第二条消息：普通对话文本，前端会正常渲染为 AI 头像的气泡
