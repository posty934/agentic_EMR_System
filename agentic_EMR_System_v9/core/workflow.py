        ...
        if extracted_data:
            ...
            for new_ent in extracted_data:
                ...
                if existing:
                    ...
                    # standard_term 是“定锚字段”，一旦已有值就不允许再拼接污染
                    ...
                    if new_ent.get("dynamic_details"):
                        ...
                        for dk, dv in new_ent.get("dynamic_details", {}).items():
                            ...
                            old_dv = existing["dynamic_details"].get(dk)
                            if not old_dv:
                                existing["dynamic_details"][dk] = dv
                            elif str(dv) not in str(old_dv):
                                existing["dynamic_details"][dk] = f"{old_dv}，{dv}"
                else:
